...
When('I click on the follow button') do
  find('button.profile-follow-button-style__StaticButton-sc-1f7o8tq-0.profile-follow-button-style__Button-sc-1f7o8tq-1.LIWnI.gasooY.btn.btn-primary').click
end
...
Then('the fund information section should display') do
  ...
  expect(page).to have_css('a.pds-info__Anchor-uku91k-0.bxdCCa',:text=>'Product Disclosure Statement')
end
...
When('I scroll down to the fund manager Q&A section') do
  page.execute_script("window.scrollBy(0,500)")
  sleep 10
end

Then('fund manager Q&A should display') do
  expect(page).to have_css('span.title__Headline-sc-1cbptdo-2.kLCZQn',:text=>'FUND MANAGER Q&A')
  expect(page).to have_css('span.title__SubHeadline-sc-1cbptdo-3.jaUZzH',:text=>"Introducing, LW's exclusive Q&A with the fund manager")
  expect(page).to have_css('div.title__Manager-sc-1cbptdo-4.lhYsRA')
  # expect(page).to have_css('a.title__NameContainer-sc-1cbptdo-5.fGRFnq')
  expect(page).to have_css('span.title__Position-sc-1cbptdo-7.fIYvQI')
  expect(page).to have_css('div.styles__SectionDetails-mwvbty-5 details__Container-sc-1ybhwum-0.jktqml.jXdETY')
end

Then('the investing with sandbox technology') do
  expect(page).to have_css('div.title__LogoContainer-seqh0u-1.hGVrGg')
  expect(page).to have_css('span',:text=>'INVESTING WITH')
  expect(page).to have_css('div.summary__Content-sc-15pynfm-1.wjKEn')
  expect(page).to have_css('div.managers__Manager-rvwxm3-1.cXGwFw')
  expect(page).to have_css('div.styles__VFlex-sc-1eh4ctj-1.kUYYyV',:text=>'INVEST WITH US')
  expect(page).to have_css('p.fund-title',:text=>'Munro Concentrated Global Growth A')
  expect(page).to have_css('span.stock-code__SSpan-f4874h-0.cnnUST',:text=>'ASX:ASX:14D')
  expect(page).to have_css('small',:text=>'Find out how you can start investing today.')
  expect(page).to have_css('[type="button"]',:text=>'HOW TO INVEST')
  expect(page).to have_css('a[href="https://develop.livewiremarkets.com/fund/munro-concentrated-global-growth-a/latest_pds"]',:text=>'Product Disclosure Statement')
  expect(page).to have_css('div.styles__VFlex-sc-1eh4ctj-1.kUYYyV',:text=>'Get In Touch')
  expect(page).to have_css('small',:text=>'Contact our team to find out more.')
  expect(page).to have_css('button.styles__TriStateButton-sc-1eh4ctj-2.hxKOAo',:text=>'CONTACT')
end

Then('disclaimer') do
  expect(page).to have_css('div.disclaimer__Content-sc-1uc3t7w-2.hZPXoU')
end
